...
//
// - v2.14 - load cms init script so we don't need to duplicate all the PHP "normalization" code in /lib/init.php
//   ...such as: setting magic_quotes_runtime to false, loading php4 compat functions for 'file_put_contents', fixing PHP SELF and SCRIPT_NAME, etc.
...
if (TinyMCE_Compressor::getParam("js")) {
  ...
  @ob_clean(); // clear output buffer of anything added by init.php, plugins, or libraries, etc (We're not expecting anything here))
...
class TinyMCE_Compressor {
   ...
   */
    ...
    for ($i = 0; $i < count($allFiles); $i++) {
      ...
      if ($file) {
        $allFilesModifiedTimes .= ',' . filemtime($file);
      }
