Class(function WorkList(workdata) {
    ...
    function _markup() {
        ...
        _prev = _elem.create('prev');
        _prev.size(50, 100).css({
            background: 'white'
        }).transform({
            y: Stage.height/2 - 50
        }).setZ(1000);

        _next = _elem.create('next');
        _next.size(50, 100).css({
            background: 'white'
        }).transform({
            x: Stage.width - 50,
            y: Stage.height/2 - 50
        ...
        _viewport = new Viewport3D(Stage.width, Stage.height);
        _viewport.element.css({
            left: "50%",
            top: "50%",
        ...
        // _viewport.perspective(5000);
        _elem.addChild(_viewport);

        // _button = _elem.create('button')
        // _button.size('100px', '100px').css({
        //     background: 'red'
        // }).setZ(10000);
        ...
        _cube = _viewport.element.create("cube");
        _cube.css({
            left: "50%",
            marginLeft: -_cubeWidth/2
            // marginTop: -_cubeHeight/2
        });
        

        // Device.browser.safari ? _cube.perspective(20000) : '';
        ...
        _front  = _cube.create("front");
        _right  = _cube.create("right");
        _left   = _cube.create("left");
        _back   = _cube.create("back");
        // _button.size(40, 40, true).setBG("assets/images/header/" + _type + ".png");

        // _front.transformPoint(0, 0, 932/2).mouseEnabled(false);
        _front.transformPoint(0, 0, _cubeWidth/2).mouseEnabled(false);
        _right.transformPoint(0, 0, _cubeWidth/2).mouseEnabled(false);
        _left.transformPoint(0, 0, _cubeWidth/2).mouseEnabled(false);
        _back.transformPoint(0, 0, _cubeWidth/2).mouseEnabled(false);
        ...
        _front.size(_cubeWidth, _cubeHeight, true).css({
            background: 'red'
        }).setZ(200);

        // _front.setBG(workdata[0].main_image[0].urlPath, "jpg");
        _front.canvas = _self.initClass(Canvas, _cubeWidth, _cubeHeight, null);
        _front.add(_front.canvas);

        _front.texture      = new Image();
        _front.texture.src  = FEDevice.getAsset(workdata[0].main_image[0].urlPath, "jpg");

        // _front.texture.onload = function () {
        //     _front.canvas.context.drawImage(_front.texture, 0, 0);
        // };


        _right.size(_cubeWidth, _cubeHeight, true).css({
            background: 'green'
        }).transform({
            x: _cubeWidth + _cubeWidth/2,
            z: -_cubeWidth/2,
            rotationY: 90
        }).setZ(100);

        _right.canvas = _self.initClass(Canvas, _cubeWidth, _cubeHeight, null);
        _right.add(_right.canvas);

        _right.texture      = new Image();
        _right.texture.src  = FEDevice.getAsset(workdata[1].main_image[0].urlPath, "jpg");

        // _right.texture.onload = function () {
        //     _right.canvas.context.drawImage(_right.texture, 0, 0);
        // };

        _left.size(_cubeWidth, _cubeHeight, true).css({
            background: 'yellow'
        }).transform({
            x: -_cubeWidth/2,
            z: -_cubeWidth - _cubeWidth/2,
            rotationY: -90
        }).setZ(100);

        _left.canvas = _self.initClass(Canvas, _cubeWidth, _cubeHeight, null);
        _left.add(_left.canvas);

        _left.texture      = new Image();
        _left.texture.src  = FEDevice.getAsset(workdata[2].main_image[0].urlPath, "jpg");

        // _left.texture.onload = function () {
        //     _left.canvas.context.drawImage(_left.texture, 0, 0);
        // };

        _back.size(_cubeWidth, _cubeHeight, true).css({
            background: 'blue'
        }).transform({
            x: _cubeWidth,
            z: -_cubeWidth - _cubeWidth,
            rotationY: -180
        }).setZ(100);
        
        _back.canvas = _self.initClass(Canvas, _cubeWidth, _cubeHeight, null);
        _back.add(_back.canvas);

        _back.texture      = new Image();
        _back.texture.src  = FEDevice.getAsset(workdata[3].main_image[0].urlPath, "jpg");

        // _back.texture.onload = function () {
        //     _back.canvas.context.drawImage(_back.texture, 0, 0);
        // };
        // _button.div.style.backgroundSize = "40px 80px";

        // console.log('Device.vendor')
        // console.log(Device.vendor)
        // console.log('Device.browser.safari')
        // console.log(Device.browser.safari)

        _cube.transformPoint(_cubeWidth/2, 0, !Device.browser.safari ? -_cubeWidth/2 : -_cubeWidth).mouseEnabled(false);
        // _cube.useMatrix3D();

        // _cube.backfaceVisibility('hidden');
        // _front.backfaceVisibility('hidden');
        // _left.backfaceVisibility('hidden');
        // _right.backfaceVisibility('hidden');
        // _back.backfaceVisibility('hidden');
    ...
    function _positionCubeBackgrounds() {
        ...
        // _front.canvas.size(_wdth, _hght);
        _front.canvas.context.drawImage(_front.texture, 0, 0, _wdth, _hght);
        
        var _one = Stage.height / 2 - _hght / 2;
        var _two = FEDevice.width / 2 - _wdth / 2;

        _front.css({
            backgroundPosition: _one + " " + _two
            // left: FEDevice.width / 2 - _wdth / 2
        });

        // _left.canvas.size(_wdth, _hght);
        _left.canvas.context.drawImage(_left.texture, 0, 0, _wdth, _hght);

        var _three = Stage.height / 2 - _hght / 2;
        var _four = FEDevice.width / 2 - _wdth / 2;

        _left.css({
            backgroundPosition: _three + " " + _four
        });

        // _right.canvas.size(_wdth, _hght);
        _right.canvas.context.drawImage(_right.texture, 0, 0, _wdth, _hght);

        var _five = Stage.height / 2 - _hght / 2;
        var _six = FEDevice.width / 2 - _wdth / 2

        _right.css({
            backgroundPosition: _five + " " + _six
        });

        // _back.canvas.size(_wdth, _hght);
        _back.canvas.context.drawImage(_back.texture, 0, 0, _wdth, _hght);

        var _seven = Stage.height / 2 - _hght / 2;
        var _eight = FEDevice.width / 2 - _wdth / 2;

        _back.css({
            backgroundPosition: _seven + " " + _eight
        });
