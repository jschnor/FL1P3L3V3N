			...
			switch (type){
				...
				if (_isLoaded === false){
					...
					_newImage.onerror = function(error){
						if (typeof callback == 'function'){
							...
							callback(false, 'AssetLoader: Image skipped (load error): '+url);
						}
					};
					...
					if (typeof callback == 'function'){
						...
						callback(false, 'AssetLoader: Image skipped (already loaded): '+url);
					}
				}
				break;
				...
				if (_isLoaded === false){
					...
					if (typeof callback == 'function'){
						...
						callback(false, 'AssetLoader: File skipped (already loaded): '+url, false);
					}
				}
				break;
