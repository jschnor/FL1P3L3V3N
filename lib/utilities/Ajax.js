Class(function Ajax() {
	...
	function _call(url, dataObj, method, callback){
		...
		_xhr.onreadystatechange = function(){
			if (_xhr.readyState == 4 ) {
				if (_xhr.status == 200){
					...
					// may have intentionally aborted
		...
		// return the object so implementations can work with it
		return _xhr;
	}


	this.call = function(url, dataObj, method, callback){
		return _call(url, dataObj, method, callback);
	};

	this.get = function(url, dataObj, callback) {
		if (typeof dataObj === 'function' && typeof callback === 'undefined'){
			return _call(url, {}, 'GET', dataObj);
		}else{
			return _call(url, dataObj, 'GET', callback);
		}
	};

	this.post = function(url, dataObj, callback){
		return _call(url, dataObj, 'POST', callback);
	};
