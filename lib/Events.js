...
// function $events() {
    ...
    var _EventsObj    = {};
    var _IEEventsObj  = {};
    var _customCallbacks = {}; // object to hold functions that have been subscribed to custom events
    var _params;
    ...
    this.subscribe = function(elem, evt, callback) {
        ...
        if (typeof evt !== undefined && typeof evt == "string") {
            if (isDOMEvent) {
                ...
                _customCallbacks[evt] = _customCallbacks[evt] || [];
                _customCallbacks[evt].push(callback);
            }
        }
    };
    ...
    this.fireEvent = function(elem, evt, params) {
        ...
        if (isDOMEvent) {
            ...
            // run the callback(s) for a custom event
            if (typeof _customCallbacks[evt] !== 'undefined'){
                for (var i = _customCallbacks[evt].length - 1; i >= 0; i--) {
                    if (typeof _customCallbacks[evt][i] == 'function') {
                        _customCallbacks[evt][i](_params);
                    }
                }
            }
        }
    ...
    this.removeEvent = function(elem, evt, callback, log) {
        if (elem.detachEvent) {
            ...
            if (elem.removeEventListener){
                ...
                if (typeof _customCallbacks[evt] !== 'undefined'){
                    for (var i = _customCallbacks[evt].length - 1; i >= 0; i--) {
                        if (_customCallbacks[evt][i] == callback) {
                            _customCallbacks[evt][i] = null;
                        }
                    }
                }
            }
