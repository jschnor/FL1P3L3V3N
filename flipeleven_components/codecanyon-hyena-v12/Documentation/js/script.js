...
$('.skinselect').bind({mouseenter:function(){
	$(this).css({'background-color':'#FFFFFF','color':'#02ba76'});
},mouseleave:function(){
	if(!$(this).hasClass('active'))$(this).css({'background-color':'#02ba76','color':'#FFFFFF'});
		
}});


$('.sidebartoggle').bind({mouseenter:function(){
	$(this).css({'background-color':'#02ba76','color':'#FFF','text-shadow':'none'});	
},mouseleave:function(){
	if(parseInt($(this).next('.sidebarcontainer').css('height'),10) === 0){
	$(this).css({'background-color':'#FFFFFF','color':'#444444','text-shadow':'1px 1px 0 #EEEEEE'});};
}});
...
$('#section1').bind('click',function(){
	var $this=$(this), $content=$this.next('.sidebarcontainer');
	if(parseInt($content.css('height'),10) === 0){
		$('div.sidebarcontainer').not($content).stop(true,true).animate({'height':'0px'},{duration:500,queue:false});
		$('div.sidebartoggle').not($this).css({'background-color':'#FFFFFF','color':'#444444','text-shadow':'1px 1px 0 #EEEEEE'});
		$content.animate({'height':'173px'},{duration:500,queue:false});
	}else{
		$content.animate({'height':'0px'},{duration:500,queue:false});		
	};
});



$('#section2').bind('click',function(){
	var $this=$(this), $content=$this.next('.sidebarcontainer');
	if(parseInt($content.css('height'),10) === 0){
		$('div.sidebarcontainer').not($content).stop(true,true).animate({'height':'0px'},{duration:500,queue:false});		
		$('div.sidebartoggle').not($this).css({'background-color':'#FFFFFF','color':'#444444','text-shadow':'1px 1px 0 #EEEEEE'});		
		$content.animate({'height':'105px'},{duration:500,queue:false});
	}else{
		$content.animate({'height':'0px'},{duration:500,queue:false});		
	};
});
...
$('#section4').bind('click',function(){
	var $this=$(this), $content=$this.next('.sidebarcontainer');
	if(parseInt($content.css('height'),10) === 0){
		$('div.sidebarcontainer').not($content).stop(true,true).animate({'height':'0px'},{duration:500,queue:false});
		$('div.sidebartoggle').not($this).css({'background-color':'#FFFFFF','color':'#444444','text-shadow':'1px 1px 0 #EEEEEE'});				
		$content.animate({'height':'40px'},{duration:500,queue:false});
	}else{
		$content.animate({'height':'0px'},{duration:500,queue:false});		
	};
});
...
$('#documenter_sidebar ol a').bind('click',function(){
	$('#documenter_sidebar ol a.current').removeClass('current');
